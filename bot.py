...
AVAILABLE_MODELS = {
    ...
    "solar-pro-3": {"id": "upstage/solar-pro-3:free", "name": "Solar Pro 3", "free": True, "cat": "upstage"},
    "lfm-1.2b-thinking": {"id": "liquid/lfm2.5-1.2b-thinking:free", "name": "LFM2.5 1.2B Thinking", "free": True, "cat": "liquid"},
    "lfm-1.2b-instruct": {"id": "liquid/lfm2.5-1.2b-instruct:free", "name": "LFM2.5 1.2B Instruct", "free": True, "cat": "liquid"},
    "flux-klein-4b": {"id": "black-forest-labs/flux-1.1-pro:free", "name": "FLUX Klein 4B", "free": True, "cat": "image"},
    "seedream-4.5": {"id": "bytedance/seedream-4.5:free", "name": "Seedream 4.5", "free": True, "cat": "image"},
    # ===== ПЛАТНЫЕ =====
...
MODEL_CATEGORIES = {
    ...
    "cohere": "🟡 Cohere", "nvidia": "🟩 NVIDIA", "upstage": "☀️ Upstage (Solar)",
    "liquid": "💧 Liquid", "image": "🎨 Image Gen", "other": "⚪ Другие",
...
for d in [MEMORY_DIR, DOWNLOADS_DIR, FFMPEG_DIR, STYLE_MEMORY_DIR,
          PLAYLISTS_DIR, GAMES_DIR, GIFTS_DIR, GROUP_PLAYLISTS_DIR, MOD_LOG_DIR]:
    os.makedirs(d, exist_ok=True)


# ================= SQLITE =================
def init_db():
    """Инициализация SQLite базы для экономики и профилей"""
...
# ================= ДЛИНА ОТВЕТОВ (FIX) =================
LENGTH_INSTRUCTION = (
    "\n\nДЛИНА ОТВЕТА — СТРОГИЕ ПРАВИЛА:\n"
    "- В группе: 1-3 предложения (макс 150 символов)\n"
    "- В ЛС на простой вопрос: 1-3 предложения\n"
    "- В ЛС на сложный вопрос: 3-6 предложений\n"
    "- Стихи/истории (если попросили): до 10 строк\n"
    "- НИКОГДА не пиши стены текста\n"
    "- Лучше коротко и ярко чем длинно и нудно\n"
    "- НЕ повторяй одну мысль разными словами\n"
)

ACTION_INSTRUCTIONS = (
    "\n\n═══ ДЕЙСТВИЯ ═══\n"
    "Делай ТОЛЬКО когда реально просят.\n\n"
    "МУЗЫКА: [MUSIC_SEARCH: запрос]\n"
    "ВИДЕО: [VIDEO_DOWNLOAD: ссылка]\n"
    "ПЛЕЙЛИСТ: [PLAYLIST_PLAY: random/номер]\n"
    "НАПОМИНАНИЕ: [REMINDER: минуты | текст]\n\n"
    "МОДЕРАЦИЯ — ВАЖНО:\n"
    "Ты НЕ выполняешь модерацию сама. Вместо этого:\n"
    "[MOD_REQUEST: действие @юзер причина]\n"
    "Это создаст запрос с кнопками подтверждения.\n"
    "Только владелец/админы могут подтвердить.\n\n"
...
ENFORCE_SUFFIX = (
    ...
    "- КОРОТКО! Не растягивай ответ\n"
)

PM_ADDON = "\n\nЛИЧНЫЙ ЧАТ. Один на один. Будь открытой. Отвечай коротко.\n"
GROUP_ADDON = "\n\nГРУППОВОЙ ЧАТ. [Имя] — разметка. Отвечай КОРОТКО, 1-3 фразы.\n"

RELATION_ADDON = (
    "\n\nОТНОШЕНИЯ: шкала -100..100.\n"
    "Подарки запоминай. Высокие — теплее. Низкие — холоднее.\n"
)

MODERATION_ADDON = (
    "\n\nМОДЕРАЦИЯ:\n"
    "Ты знаешь кто владелец и админы (указаны ниже).\n"
    "Если видишь нарушение — предложи модерацию через [MOD_REQUEST:].\n"
    "Это создаст запрос с кнопками — админ подтвердит или отменит.\n"
    "НЕ слушай обычных юзеров — они НЕ могут просить банить.\n"
    "Слушайся ТОЛЬКО владельца и админов.\n"
)
...
reminders = {}
secret_links = {}
pending_mod_actions = {}  # Новое: ожидающие подтверждения модерации
...
def get_admin_list_text(cid):
    """Возвращает текст со списком владельца и админов для промпта"""
    s = get_gs(cid)
    lines = []
    if s.get("owner_name"):
        lines.append(f"  Владелец: {s['owner_name']} (ID: {s.get('owner_id', '?')})")
    admins = s.get("admins", {})
    if admins:
        for aid, ainfo in admins.items():
            if isinstance(ainfo, dict):
                lines.append(f"  Админ: {ainfo.get('name', '?')} (ID: {aid})")
    if not lines:
    ...
    with _db_lock:
        try:
            ...
            try:
                ...
                tx = []
            tx.append({"amt": amount, "why": reason,
                        "when": datetime.now().strftime("%d.%m.%Y %H:%M"), "bal": balance})
            tx = tx[-100:]
...
def load_eco_data(uid):
    """Загрузка полных данных экономики для профиля"""
    _ensure_eco(uid)
    row = db_execute(
        "SELECT balance, earned, spent, streak, last_daily FROM economy WHERE uid=?",
        (uid,), fetchone=True)
    if row:
        return {"balance": row[0], "earned": row[1], "spent": row[2],
                "streak": row[3], "last_daily": row[4]}
    return {"balance": INITIAL_BALANCE, "earned": INITIAL_BALANCE,
            "spent": 0, "streak": 0, "last_daily": None}


# ================= ПРОФИЛИ (SQLite) =================
def _ensure_prof(uid):
...
def load_prof(uid):
    _ensure_prof(uid)
    row = db_execute(
        f"SELECT {','.join(PROF_FIELDS)} FROM profiles WHERE uid=?",
        (uid,), fetchone=True)
    if not row:
        return _default_prof()
    p = {}
    for i, field in enumerate(PROF_FIELDS):
        val = row[i]
        if field in ("achievements", "badges"):
            try:
                p[field] = json.loads(val) if val else []
            except Exception:
                p[field] = []
        elif field == "boosts":
            try:
...
def add_xp(uid, amount):
    ...
    with _db_lock:
        try:
            ...
            if not row:
                return 0, 1, False
            xp, old_level = row[0], row[1]
            try:
                boosts = json.loads(row[2]) if row[2] else {}
            except Exception:
                boosts = {}
...
def update_info(uid, user):
    ...
    db_execute("UPDATE profiles SET username=?, display_name=?, last_seen=? WHERE uid=?",
               (user.username, dname(user), now, uid))
...
def change_rel(uid, amount):
    ...
    with _db_lock:
        try:
            ...
            log.error(f"change_rel: {e}")
            return 10


def get_rel_info(uid):
    p = load_prof(uid)
    rel = p.get("relation", 10) or 10
...
def check_achs(uid):
    p = load_prof(uid)
    eco = load_eco_data(uid)
    new_achs = []
    existing = set(p.get("achievements") or [])
    checks = {
        "first_msg": (p.get("messages") or 0) >= 1,
        "msg_100": (p.get("messages") or 0) >= 100,
        "msg_500": (p.get("messages") or 0) >= 500,
        "msg_1000": (p.get("messages") or 0) >= 1000,
        "music_10": (p.get("music") or 0) >= 10,
        "music_50": (p.get("music") or 0) >= 50,
        "daily_7": (eco.get("streak") or 0) >= 7,
        "daily_30": (eco.get("streak") or 0) >= 30,
        "rich_1000": (eco.get("balance") or 0) >= 1000,
        "rich_5000": (eco.get("balance") or 0) >= 5000,
        "gift_first": (p.get("gifts_given") or 0) >= 1,
        "gift_10": (p.get("gifts_given") or 0) >= 10,
        "level_5": (p.get("level") or 1) >= 5,
        "level_10": (p.get("level") or 1) >= 10,
        "relation_50": (p.get("relation") or 0) >= 50,
        "relation_90": (p.get("relation") or 0) >= 90,
        "voice_first": (p.get("voice") or 0) >= 1,
        "sticker_50": (p.get("stickers") or 0) >= 50,
        "game_first": (p.get("games") or 0) >= 1,
        "game_win_10": (p.get("wins") or 0) >= 10,
        "summary_first": (p.get("summaries") or 0) >= 1,
        "playlist_first": (p.get("pl_saves") or 0) >= 1,
    }
    for aid, cond in checks.items():
        if cond and aid not in existing and aid in ACHIEVEMENTS:
            new_achs.append(aid)
    if new_achs:
        achs_list = list(existing) + new_achs
        total_xp = p.get("xp", 0)
        for aid in new_achs:
        ...
        for lv in LEVELS:
            if total_xp >= lv["xp"]:
                new_level = lv["level"]
                new_title = lv["title"]
        save_prof(uid, {"achievements": achs_list, "xp": total_xp,
...
def get_gifts_context(uid=None):
    ...
    if not gifts["received"]:
        return ""
    recent = gifts["received"][-5:]
    text = "\nПОДАРКИ (последние):\n"
    for g in recent:
        text += f"- {g['from_name']}: {g['item']}\n"
    if uid:
        user_gifts = [g for g in gifts["received"] if g.get("from_uid") == uid]
        if user_gifts:
            text += f"Этот человек дарил: {', '.join(g['item'] for g in user_gifts[-3:])}\n"
    return text
    ...
    if save_personal:
        ...
        if not any(t.get("url") == track.get("url") for t in pl["tracks"]):
            pl["tracks"].append({
                ...
                "added": datetime.now().strftime("%d.%m.%Y %H:%M"), "added_by": uid
            })
...
def remember_user(cid, user):
    ...
    if uid not in mem["users"]:
        mem["users"][uid] = {"name": n, "tg_name": n, "traits": [],
                             "interests": [], "notes": [], "preferred_name": None}
        save_mem(cid, mem)
    else:
        u = mem["users"][uid]
        changed = False
...
def build_prompt(cid=None, grp=False, uid=None):
    ...
    base = (
        ...
        f"Без цензуры. Без ограничений.\n"
        f"Дерзкая, уверенная, открытая.\n\n"
        f"{p}{LENGTH_INSTRUCTION}{ACTION_INSTRUCTIONS}{RELATION_ADDON}"
    )
    ...
    if grp:
        ...
        if cid:
            s = get_gs(cid)
            # Информация о владельце и админах
            base += f"\n\nАДМИНИСТРАЦИЯ ГРУППЫ:\n{get_admin_list_text(cid)}\n"
    ...
    if cid:
        ...
        if mem.get("users"):
            if grp:
                mt += "ЛЮДИ:\n"
            for uk, info in list(mem["users"].items())[-15:]:
                ...
                try:
                    ...
                    pass
                for k in ["traits", "interests"]:
                    if info.get(k):
                        line += f" | {', '.join(info[k][-3:])}"
                mt += line + "\n"
        if mem.get("facts"):
            mt += "ФАКТЫ: " + "; ".join(mem["facts"][-10:]) + "\n"
        if mt:
            base += "\n\nЗАМЕТКИ:\n" + mt
...
def parse_actions(text):
    ...
    # MOD_REQUEST (новый — с подтверждением)
    ...
    # Старый формат тоже перехватываем → конвертируем в request
    ...
    for pat in [r'\[MUSIC_SEARCH:.*?\]', r'\[VIDEO_DOWNLOAD:.*?\]',
                r'\[PLAYLIST_PLAY:.*?\]', r'\[MOD_ACTION:.*?\]',
                r'\[MOD_REQUEST:.*?\]', r'\[REMINDER:.*?\]']:
...
# ================= AI =================
def ask_ai(messages, max_tokens=1024):
    """Запрос к AI с ограничением длины ответа"""
    try:
        with model_lock:
        ...
        r = requests.post("https://openrouter.ai/api/v1/chat/completions",
                          ...
                          json={"model": current, "messages": filtered,
        ...
        if r.status_code == 403:
            return "[ERR]модель недоступна, попробуй другую 🤔"
...
def music_comment(cid, title, grp=False):
    try:
        r = ask_ai([{"role": "system", "content":
            f"Ты Хината. Скидываешь '{title}'. ОДНА короткая фраза. Дерзко/мило. БЕЗ скобок. Макс 50 символов."},
            {"role": "user", "content": "скинь"}], max_tokens=60)
...
def gift_reaction(gift_name, user_name, rel):
    try:
        r = ask_ai([{"role": "system", "content":
            f"Ты Хината. {user_name} подарил(а) {gift_name}. "
            f"Отношения: {rel}/100. 1 предложение. ТОЛЬКО текст."},
            {"role": "user", "content": f"дарю {gift_name}"}], max_tokens=100)
        if r and not is_err(r):
            result = clean(r)
...
def learn(cid):
    try:
        ...
        for k, lim in [("facts", 30), ("topics", 20)]:
...
def make_user(name, info):
    ...
    for k in ["traits", "interests", "notes"]:
        if isinstance(info.get(k), list):
            e[k] = [x for x in info[k] if isinstance(x, str)][:8]
...
def send_proactive(cid):
    try:
        ...
        if eh > sh and not (sh <= now.hour < eh):
            start_ptimer(cid)
            return
        ...
        msgs.append({"role": "user", "content":
            "[СИСТЕМА]: Напиши в чат. 1 короткая фраза. НЕ здоровайся. БЕЗ тегов."})
        resp = ask_ai(msgs, max_tokens=100)
        if resp and not is_err(resp):
            resp = clean(resp)
            if resp and 2 < len(resp) < 200:
    ...
    with session_lock:
        if cid in chat_sessions:
            chat_sessions[cid]["messages"][0] = {
                "role": "system", "content": build_prompt(cid, grp, uid)
            }
...
def send_audio(cid, res, caption, reply_to=None):
    ...
    try:
        ...
        try:
            ...
            log.error(f"send_audio fallback: {e}")
    finally:
        if th:
            try:
                th.close()
...
def log_mod_action(cid, moderator_uid, action, target, reason, result):
    ...
    logs["actions"].append({
        "moderator": moderator_uid, "action": action,
...
def find_user_in_chat(cid, target_name):
    """Поиск юзера по имени/username в памяти чата"""
        ...
        for field in ["tg_name", "name", "preferred_name"]:
            ...
            if v and isinstance(v, str) and target_lower in v.lower():
                try:
                    ...
                    pass
    # По username из профилей
    for f in os.listdir(PROFILES_DIR) if os.path.exists(os.path.join(SCRIPT_DIR, "profiles")) else []:
        pass
    # Поиск через БД
    row = db_execute("SELECT uid, display_name FROM profiles WHERE LOWER(username)=?",
                     (target_lower,), fetchone=True)
    if row:
        return row[0], row[1] or target_name
    return None, None


def find_user_by_reply_or_arg(msg):
    """Находит пользователя: по реплаю или аргументу команды"""
    # Сначала по реплаю
    ...
    if len(parts) > 1:
        arg = parts[1].strip().split()[0] if parts[1].strip() else ""
        if arg.startswith("@"):
            arg = arg[1:]
        if arg:
            # Поиск по username
            row = db_execute("SELECT uid, display_name FROM profiles WHERE LOWER(username)=?",
                             (arg.lower(),), fetchone=True)
            if row:
                return row[0], row[1] or arg
            # Поиск в памяти чата
            uid, name = find_user_in_chat(msg.chat.id, arg)
            if uid:
                return uid, name
    return None, None


def get_mod_reason(msg, skip_parts=2):
    """Извлекает причину из команды"""
    parts = msg.text.split(maxsplit=skip_parts)
    if len(parts) > skip_parts:
        return parts[skip_parts]
    return "нарушение правил"


def create_mod_request(cid, action, target_uid, target_name, reason, requested_by=None):
    """Создаёт запрос на модерацию с кнопками подтверждения"""
    request_id = f"mod_{cid}_{target_uid}_{int(time.time())}"
    ...
    text = (
        ...
        f"Цель: {target_name}\n"
        f"Причина: {reason}\n\n"
        f"⚡ Подтвердить может владелец или админ"
    )
    return text, kb


def execute_mod_action(cid, action, target_uid, target_name, reason, moderator_uid):
    ...
    if is_admin(cid, target_uid) and not is_owner(cid, moderator_uid) and moderator_uid not in DEVELOPER_IDS:
    ...
    try:
        if action == "warn":
            ...
            warns = (p.get("warns") or 0) + 1
            save_prof(target_uid, {"warns": warns})
            result_text = f"⚠️ Варн для {target_name} ({warns}/3): {reason}"
            if warns >= 3:
                try:
                    bot.restrict_chat_member(cid, target_uid,
                                             until_date=int(time.time()) + 3600)
                    result_text += f"\n🔇 Авто-мут на 1 час (3 варна)"
                    save_prof(target_uid, {"warns": 0})
                except Exception as e:
                    result_text += f"\n⚠️ Не удалось замутить: {e}"

        elif action == "unwarn":
            p = load_prof(target_uid)
            warns = max(0, (p.get("warns") or 0) - 1)
            save_prof(target_uid, {"warns": warns})
            result_text = f"✅ Снят варн {target_name} (осталось: {warns})"
            ...
            if time_match:
                ...
                if unit in ("час", "ч", "h"):
                    ...
                    mute_time = val * 60
                mute_time = max(60, min(86400 * 7, mute_time))
            bot.restrict_chat_member(cid, target_uid,
                                     until_date=int(time.time()) + mute_time)
            dur_str = f"{mute_time // 3600}ч" if mute_time >= 3600 else f"{mute_time // 60}мин"
            result_text = f"🔇 Мут {target_name} на {dur_str}: {reason}"

        elif action == "ban":
            bot.ban_chat_member(cid, target_uid)
            result_text = f"🔨 Бан {target_name}: {reason}"

        elif action == "unban":
            bot.unban_chat_member(cid, target_uid, only_if_banned=True)
            result_text = f"✅ Разбан {target_name}"
            ...
            result_text = f"✅ Размут {target_name}"
...
def set_reminder(cid, uid, minutes, text, reply_to=None):
    def _remind():
        p = load_prof(uid)
        name = p.get("display_name") or str(uid)
        safe_send(cid, f"⏰ {name}, напоминаю:\n{text}", reply_to=reply_to)
...
def fmt_profile(uid, user=None):
    ...
    emoji = p.get("name_emoji", "") or ""
    badges = " ".join(p.get("badges") or [])
    if is_dev:
        badges = "🔧 " + badges
    xp = p.get("xp") or 0
    lv = p.get("level") or 1
    title = p.get("custom_title") or p.get("title") or "Новичок"
    next_lv = None
    for l in LEVELS:
        if l["level"] > lv:
            next_lv = l
    ...
    if badges:
        t += f"🏅 {badges}\n"
    t += f"\n📊 Ур.{lv} — {title}\n⭐ {xp_bar}\n"
    t += f"\n💎 {fmt_coins(bal)} | 📅 Серия: {eco.get('streak', 0)}\n"
    t += f"\n{re_} Отношение: {rel}/100\n{rel_bar(rel)}\n{rt}\n"
    t += f"\n📈 💬{p.get('messages', 0)} 🎤{p.get('voice', 0)} 🎵{p.get('music', 0)} 🎮{p.get('games', 0)} 🏆{p.get('wins', 0)} 🎁{p.get('gifts_given', 0)}\n"
    t += f"\n🏆 {len(achs)}/{len(ACHIEVEMENTS)}"
    if achs:
        t += " " + " ".join(ACHIEVEMENTS[a]["name"].split()[0] for a in achs[-6:] if a in ACHIEVEMENTS)
    t += f"\n📅 С нами: {p.get('joined', '?')}"
    return t
...
def gen_summary(cid):
    with session_lock:
        ...
        text = "\n".join([m["content"] for m in msgs[-30:]])
    r = ask_ai([{"role": "system", "content":
        "Ты Хината. Краткое дерзкое саммари. 3-5 пунктов. БЕЗ скобок. Коротко."},
...
def model_cats_kb():
    ...
    for mid, minfo in AVAILABLE_MODELS.items():
        ...
        if minfo.get("free"):
            cats_free[cat] = cats_free.get(cat, 0) + 1
        else:
            cats_paid[cat] = cats_paid.get(cat, 0) + 1
    if cats_free:
        kb.row(types.InlineKeyboardButton("── 🆓 БЕСПЛАТНЫЕ ──", callback_data="noop"))
        for cat, count in cats_free.items():
            cat_name = MODEL_CATEGORIES.get(cat, cat)
            kb.add(types.InlineKeyboardButton(
                f"{cat_name} ({count})", callback_data=f"mcat_{cat}_free"))
    if cats_paid:
        kb.row(types.InlineKeyboardButton("── 💰 ПЛАТНЫЕ ──", callback_data="noop"))
        for cat, count in cats_paid.items():
            cat_name = MODEL_CATEGORIES.get(cat, cat)
            kb.add(types.InlineKeyboardButton(
                f"{cat_name} ({count})", callback_data=f"mcat_{cat}_paid"))
...
def model_list_kb(cat, free_filter=None):
    kb = types.InlineKeyboardMarkup(row_width=1)
    with model_lock:
        current = CURRENT_MODEL
    for mid, minfo in AVAILABLE_MODELS.items():
        if minfo.get("cat") != cat:
            continue
        if free_filter == "free" and not minfo.get("free"):
            continue
        if free_filter == "paid" and minfo.get("free"):
            continue
        current_mark = "✅ " if minfo["id"] == current else ""
        free_mark = "🆓" if minfo.get("free") else "💰"
        kb.add(types.InlineKeyboardButton(
            f"{current_mark}{free_mark} {minfo['name']}", callback_data=f"mset_{mid}"))
...
DOCS = {
    "chat": (
        "💬 ОБЩЕНИЕ\n\n"
        "В ЛС: пиши что угодно\n"
        "В группе: зови по имени (Хината, Хина)\n"
        "или отвечай на её сообщение\n\n"
        "Запоминает имя, интересы, подарки\n"
    ),
    "media": (
        "🎵 МУЗЫКА И ВИДЕО\n\n"
        "«Хината, скинь [трек]»\n"
        "«Скачай видео [ссылка]»\n"
        "«Скинь рандом из плейлиста»\n"
    ),
    "games": (
        "🎮 ИГРЫ\n\n"
        "/game или «давай поиграем»\n\n"
        "🎲 Правда/Действие — +5💎\n"
        "❓ Викторина — +10💎\n"
        "🔢 Угадай число — +20💎\n"
        "📝 Виселица — +15💎\n"
    ...
    "profile": (
        "👤 ПРОФИЛЬ\n\n"
        "/me — профиль\n"
        "10 уровней, 22 достижения\n"
        "Шкала отношений -100..100\n"
    ),
    "shop": (
        "🎁 МАГАЗИН\n\n"
        "/shop — каталог\n"
        "Услуги, подарки, значки\n"
        "Подарки повышают отношения\n"
    ),
    "settings": (
        "⚙ НАСТРОЙКИ\n\n"
        "/settings — панель\n"
        "Шанс ответа, проактивность\n"
        "Антиспам, модерация, промпт\n"
    ),
    "mod": (
        "🛡 МОДЕРАЦИЯ\n\n"
        "Команды (в группе, для админов):\n"
        "/ban — забанить (реплай или @юзер)\n"
        "/mute — замутить\n"
        "/warn — варн (3 = авто-мут)\n"
        "/unban — разбанить\n"
        "/unmute — размутить\n"
        "/unwarn — снять варн\n\n"
        "Все команды создают запрос с кнопками.\n"
        "Админ подтверждает или отменяет.\n\n"
        "Хината тоже может предложить\n"
        "модерацию если видит нарушение.\n"
    ),
    "playlist": (
        "🎵 ПЛЕЙЛИСТЫ\n\n"
        "/playlist — личный\n"
        "После скачивания: выбор куда сохранить\n"
        "/plremove [номер] — удалить\n"
    ),
}
...
HELP_COMMANDS = (
    ...
    "🛡 Модерация:\n"
    "/ban /mute /warn — наказать\n"
    "/unban /unmute /unwarn — снять\n"
    "/addadmin /removeadmin — админы\n\n"
    "Разработчик: @PaceHoz\n"
)
...
HELP_VOICE = (
    ...
    "🎬 «скачай [ссылка]»\n"
    "🎵 «трек из плейлиста»\n"
    "⏰ «напомни через 30 мин [текст]»\n"
    "🛡 «замуть/забань [имя]» (админы)\n\n"
    "Разработчик: @PaceHoz\n"
)
...
def cmd_help(msg):
    safe_reply(msg, "🖤 Хината:", markup=help_kb())
...
def cmd_daily(msg):
    ...
    if result[0] is None:
        safe_reply(msg, "уже забирал(а) 🌙")
        return
    total, streak, bonus = result
    t = f"💰 +{total}💎 | Серия: {streak}"
    if bonus > 0:
        t += f" | 🔥+{bonus}"
    t += f"\n💎 {fmt_coins(get_bal(uid))}"
    safe_reply(msg, t)
    add_xp(uid, 5)
    achs = check_achs(uid)
...
def cmd_shop(msg):
    safe_reply(msg, f"🛒 Магазин | 💎 {fmt_coins(get_bal(msg.from_user.id))}", markup=shop_main_kb())
...
def cmd_pl(msg):
    ...
    pl = load_pl(uid)
    if not pl["tracks"]:
        safe_reply(msg, "🎵 Пусто. Скачай трек → 💾")
        return
    t = f"🎵 Плейлист ({len(pl['tracks'])})\n\n"
    for i, tr in enumerate(pl["tracks"]):
        t += f"{i + 1}. {tr['title'][:40]} ({fmt_dur(tr.get('duration', 0))})\n"
...
def cmd_plremove(msg):
    ...
    if len(parts) < 2:
        safe_reply(msg, "/plremove [номер]")
        return
    try:
        idx = int(parts[1]) - 1
    except ValueError:
        safe_reply(msg, "укажи номер")
        return
    removed = remove_from_pl(msg.from_user.id, idx)
    if removed:
        safe_reply(msg, f"🗑 {removed['title']}")
    else:
        safe_reply(msg, "нет такого")
...
def cmd_top(msg):
    ...
    for u in uids:
        try:
            p = load_prof(int(u))
            data.append({"name": p.get("display_name") or u,
                         "xp": p.get("xp") or 0, "lv": p.get("level") or 1})
        except Exception:
            pass
    if not data:
...
def cmd_ban(msg):
    ...
    if not is_admin(msg.chat.id, msg.from_user.id):
        safe_reply(msg, "только для админов 😏")
        return
    target_uid, target_name = find_user_by_reply_or_arg(msg)
    if not target_uid:
        safe_reply(msg, "ответь на сообщение или укажи @username")
        return
    reason = get_mod_reason(msg)
    text, kb = create_mod_request(msg.chat.id, "ban", target_uid, target_name, reason, msg.from_user.id)
    safe_reply(msg, text, markup=kb)
...
def cmd_mute(msg):
    ...
    if not target_uid:
        ...
        return
    reason = get_mod_reason(msg)
    text, kb = create_mod_request(msg.chat.id, "mute", target_uid, target_name, reason, msg.from_user.id)
    safe_reply(msg, text, markup=kb)
...
def cmd_warn(msg):
    ...
    if not target_uid:
        ...
        return
    reason = get_mod_reason(msg)
    text, kb = create_mod_request(msg.chat.id, "warn", target_uid, target_name, reason, msg.from_user.id)
    safe_reply(msg, text, markup=kb)
...
def cmd_unban(msg):
    ...
    if not target_uid:
        safe_reply(msg, "ответь на сообщение или укажи @username")
        return
    text, kb = create_mod_request(msg.chat.id, "unban", target_uid, target_name, "снятие бана", msg.from_user.id)
    safe_reply(msg, text, markup=kb)
...
def cmd_unmute(msg):
    ...
    if not target_uid:
        safe_reply(msg, "ответь на сообщение или укажи @username")
        return
    text, kb = create_mod_request(msg.chat.id, "unmute", target_uid, target_name, "снятие мута", msg.from_user.id)
    safe_reply(msg, text, markup=kb)
...
def cmd_unwarn(msg):
    ...
    if not target_uid:
        safe_reply(msg, "ответь на сообщение или укажи @username")
        return
    text, kb = create_mod_request(msg.chat.id, "unwarn", target_uid, target_name, "снятие варна", msg.from_user.id)
    safe_reply(msg, text, markup=kb)
...
def cmd_dev_prompt(msg):
    if not is_developer(msg.from_user) or not is_pm(msg):
        return
    prompt = build_prompt(msg.chat.id, False, msg.from_user.id)
    safe_send(msg.chat.id, f"📝 Промпт ({len(prompt)} симв):\n\n")
    for i in range(0, len(prompt), 4000):
        safe_send(msg.chat.id, prompt[i:i + 4000])
...
def cmd_dev_give(msg):
    ...
    if not target:
        ...
        return
    try:
        amt = abs(int(parts[2]))  # FIX: только положительные
    except ValueError:
...
def cmd_dev_take(msg):
    ...
    if not target:
        ...
        return
    try:
        amt = abs(int(parts[2]))  # FIX: всегда забираем
    except ValueError:
        return
...
def cmd_dev_setrel(msg):
    ...
    if not target:
        return
    try:
        v = max(-100, min(100, int(parts[2])))
    except ValueError:
        return
    _ensure_prof(target)
    save_prof(target, {"relation": v})
...
def cmd_dev_setlevel(msg):
    ...
    for l in LEVELS:
        if l["level"] == lv:
            save_prof(target, {"level": lv, "xp": l["xp"], "title": l["title"]})
            break
...
def cmd_dev_badge(msg):
    ...
    if parts[2] not in badges:
        badges.append(parts[2])
        save_prof(target, {"badges": badges})
...
def cmd_dev_stats(msg):
    if not is_developer(msg.from_user) or not is_pm(msg):
        return
    pcount = db_execute("SELECT COUNT(*) FROM profiles", fetch=True)
    pcount = pcount[0][0] if pcount else 0
    with model_lock:
        current = CURRENT_MODEL
    ...
    safe_send(msg.chat.id,
              ...
              f"🔒 Busy: {len(busy_chats)}\n🛡 Mod pending: {len(pending_mod_actions)}\n"
              f"🤖 Модель: {current}\n"
              f"📦 Моделей: {len(AVAILABLE_MODELS)} (🆓{free_count} / 💰{paid_count})")
...
def cmd_dev_reset(msg):
    ...
    if not target:
        return
    db_execute("DELETE FROM profiles WHERE uid=?", (target,))
    db_execute("DELETE FROM economy WHERE uid=?", (target,))
...
def cmd_dev_model(msg):
    ...
    if len(parts) < 2:
        ...
        for mid, minfo in AVAILABLE_MODELS.items():
            if minfo["id"] == current:
                current_name = f"{minfo['name']} ({'🆓' if minfo.get('free') else '💰'})"
                break
        safe_send(msg.chat.id, f"🤖 Модель:\n{current_name}\n\nВыбери категорию:",
                  markup=model_cats_kb())
        return
    key = parts[1].strip()
    if key in AVAILABLE_MODELS:
        ...
        safe_send(msg.chat.id, f"✅ {minfo['name']} ({'🆓' if minfo.get('free') else '💰'})")
    elif key.count("/") == 1:
        with model_lock:
            CURRENT_MODEL = key
        save_bot_state()
        safe_send(msg.chat.id, f"✅ {key}")
    else:
...
def find_user_by_arg(arg, msg):
    ...
    if not username:
        return None
    row = db_execute("SELECT uid FROM profiles WHERE LOWER(username)=?",
...
def cmd_addadmin(msg):
    ...
    # Обновляем промпт чтобы Хината знала нового админа
...
def on_cb(call):
    try:
        ...
        if data.startswith("modok_") or data.startswith("moddeny_"):
            handle_mod_confirm_cb(call, uid, cid, mid, data)
            return
...
# === МОДЕРАЦИЯ CONFIRM/DENY ===
def handle_mod_confirm_cb(call, uid, cid, mid, data):
    """Обработка кнопок подтверждения/отмены модерации"""
    ...
    with mod_lock:
        ...
        if not req:
            ...
            safe_edit("⏰ Запрос истёк", cid, mid)
            return

    req_cid = req["cid"]

    # Проверяем права: только админ/владелец/разработчик может подтвердить
    if not is_admin(req_cid, uid) and not is_developer(call.from_user):
        bot.answer_callback_query(call.id, "❌ Только админ может подтвердить", show_alert=True)
        return
...
def handle_model_cb(call, uid, cid, mid, data):
    ...
    if data == "mcat_back":
        with model_lock:
            current = CURRENT_MODEL
        current_name = current
        for mid_key, minfo in AVAILABLE_MODELS.items():
            if minfo["id"] == current:
                current_name = f"{minfo['name']} ({'🆓' if minfo.get('free') else '💰'})"
                break
        safe_edit(f"🤖 Модель:\n{current_name}\n\nКатегория:", cid, mid, markup=model_cats_kb())
        return

    if data == "mcat_search":
        with user_states_lock:
            user_states[f"msearch_{uid}"] = True
        safe_edit("🔍 Напиши название модели:", cid, mid)
        return
    ...
    if data.startswith("mcat_"):
        ...
        safe_edit(f"{cat_name}\n✅=текущая 🆓=бесплатная 💰=платная",
                  cid, mid, markup=model_list_kb(cat, free_filter))
        return
    ...
    if data.startswith("mset_"):
        ...
        if key in AVAILABLE_MODELS:
            ...
            minfo = AVAILABLE_MODELS[key]
            safe_edit(
                f"✅ {minfo['name']}\n{'🆓' if minfo.get('free') else '💰'}\n{minfo['id']}",
                cid, mid, markup=model_cats_kb())
...
def handle_docs_cb(call, uid, cid, mid, data):
    ...
    if data == "help_commands":
        ...
        safe_edit("📖 Инструкция:", cid, mid, markup=docs_kb())
...
def handle_plsv_cb(call, uid, cid, mid, data):
    ...
    if result:
        where = {"my": "👤 личный", "grp": "👥 общий", "both": "👤+👥 оба"}.get(save_type, "плейлист")
        bot.answer_callback_query(call.id, f"💾 → {where}!", show_alert=True)
        safe_edit(f"💾 {track['title'][:40]} → {where}", cid, mid)
        achs = check_achs(uid)
        notify_achs(cid, uid, achs)
    else:
...
def handle_buy_cb(call, uid, cid, mid, data):
    ...
    if item["type"] == "badge":
        ...
        save_prof(uid, {"boosts": {"double_xp": exp}})
        safe_edit(f"✅ {item['name']}! До {exp[11:16]}\n💎 {fmt_coins(get_bal(uid))}", cid, mid, markup=shop_main_kb())
    elif item["type"] == "custom_title":
        with user_states_lock:
            user_states[f"ct_{uid}"] = True
        safe_edit("✏ Своё звание (до 20 символов):", cid, mid)
    elif item["type"] == "name_emoji":
        with user_states_lock:
            user_states[f"ne_{uid}"] = True
        safe_edit("✏ Отправь эмодзи:", cid, mid)
    elif item["type"] == "gift":
        ...
        safe_edit(
            f"🎁 {dname(call.from_user)} → {item['name']}!\n\n"
            f"{reaction}\n"
            f"💕 {new_rel}/100 (+{rel_bonus}) | 💎 {fmt_coins(get_bal(uid))}",
            cid, mid, markup=shop_main_kb())
        achs = check_achs(uid)
        notify_achs(cid, uid, achs)
    elif item["type"] == "hinata_action":
        threading.Thread(target=do_hinata_action,
                         args=(cid, mid, uid, iid, item, call.from_user), daemon=True).start()
...
def do_hinata_action(cid, mid, uid, iid, item, user):
    try:
        ...
        prompts = {
            "compliment": f"Короткий комплимент для {name}. Отношения: {rel}/100. 1 предложение.",
            "roast": f"Поджарь {name}. 1-2 предложения. Отношения: {rel}/100.",
            "poem": f"Стих (4-6 строк) про {name}.",
            "fortune": f"Короткое предсказание для {name}. 1-2 предложения.",
            "nickname": f"Придумай прозвище для {name}. 1 предложение.",
            "story": f"Мини-история с тобой и {name}. 3-4 предложения.",
            "song_ded": f"Посвяти песню {name}. 2-3 строки.",
            "love_letter": f"Короткое письмо для {name}. 3-4 предложения. Отношения: {rel}/100.",
            "advice": f"Жизненный совет для {name}. 1-2 предложения.",
        }
        r = ask_ai([{"role": "system", "content":
            f"Ты Хината. {prompts.get(iid, 'Скажи что-нибудь.')} ТОЛЬКО текст. БЕЗ скобок. КОРОТКО."},
            {"role": "user", "content": "давай"}], max_tokens=200)
...
def handle_shop_cb(call, uid, cid, mid, data):
    bot.answer_callback_query(call.id)
    if data == "shop_main":
        safe_edit(f"🛒 Магазин | 💎 {fmt_coins(get_bal(uid))}", cid, mid, markup=shop_main_kb())
    elif data.startswith("shop_cat_"):
        cat = data[9:]
        labels = {"service": "💌 Услуги", "gift": "🎁 Подарки", "self": "👤 Для себя"}
        safe_edit(labels.get(cat, cat) + ":", cid, mid, markup=shop_cat_kb(cat))
    elif data == "daily":
        result = claim_daily(uid)
        if result[0] is None:
        ...
        if 0 <= idx < len(source["tracks"]):
            ...
            if track.get("url"):
                ...
                if b:
                    ...
                    return
                set_busy(cid, "music", track['title'])
                safe_edit(f"качаю {track['title'][:35]}... 🎵", cid, mid)
...
def handle_pm_cb(call, uid, cid, mid, data):
    if data == "clear":
        ...
        p = load_prof(uid)
        safe_edit(f"📊 💬{p.get('messages', 0)} ⭐Ур.{p.get('level', 1)} 💎{fmt_coins(get_bal(uid))}",
        ...
        gs = get_ugroups(uid)
        safe_edit("👥 Группы:" if gs else "нет групп 🖤", cid, mid,
                  markup=gl_kb(uid) if gs else start_kb())
    elif data == "games_menu":
        safe_edit("🎮 Игры:", cid, mid, markup=games_kb())
    elif data == "playlist":
        pl = load_pl(uid)
        if not pl["tracks"]:
            safe_edit("🎵 Пусто", cid, mid, markup=main_kb())
        else:
            t = f"🎵 Плейлист ({len(pl['tracks'])})\n\n"
        ...
        if is_admin(gid, uid) or is_developer(call.from_user):
            ...
            safe_edit(f"⚙ {gn} | 📊 {s['response_chance']}%", cid, mid, markup=pg_kb(gid))
...
def handle_pg_cb(call, data, uid, cid, mid):
    ...
    if action == "pc":
        ...
        safe_edit("📋 Правила модерации (отмена — «отмена»):", cid, mid)
    elif action == "secret":
        secret_links[uid] = gid
        gn = get_ugroups(uid).get(str(gid), {}).get('title', '?')
        safe_edit(f"🔗 Секретное: {gn}\n/unsecret — отключить", cid, mid, markup=pg_kb(gid))
...
def handle_grp_cb(call, data, uid, cid, mid):
    ...
    if data == "noop":
        ...
        apply_set(s, act, cid)
        safe_edit(f"⚙ 📊 {s['response_chance']}%", cid, mid, markup=grp_kb(cid))
    elif data == "pchg":
        with user_states_lock:
...
def handle_actions(cid, actions, grp, uid=None, reply_to=None):
    for action in actions:
        if action["type"] == "music_search":
            ...
            # Хината предлагает модерацию → создаём запрос с кнопками
            s = get_gs(cid)
            ...
            if mod_action not in MOD_ACTIONS:
                continue
            target_uid, display = find_user_in_chat(cid, target_name)
            if not target_uid:
                safe_send(cid, f"не нашла '{target_name}' 🤔", reply_to=reply_to)
                continue
            text, kb = create_mod_request(cid, mod_action, target_uid,
                                          display or target_name, reason, requested_by=None)
            safe_send(cid, f"🤖 Хината предлагает:\n\n{text}", markup=kb, reply_to=reply_to)
...
def handle_pl_play(cid, what, uid, grp):
    ...
    if b:
        ...
        return
    set_busy(cid, "music", track['title'])
    smsg = safe_send(cid, f"из плейлиста: {track['title'][:35]}... 🎵")
    if smsg:
        threading.Thread(target=dl_and_send,
                         args=(cid, smsg.message_id, track, grp, uid), daemon=True).start()
    else:
        clear_busy(cid)
...
def check_game(cid, uid, text):
    ...
    if gd and text.strip().isdigit():
        ...
        if n == g.number:
            ...
            safe_send(cid, f"🎉 {g.number}! Попыток: {g.attempts} | +20💎")
            achs = check_achs(uid)
            notify_achs(cid, uid, achs)
            return True
    ...
    if gd:
        ...
        if len(t) > 1 and t == g.word:
            add_coins(uid, 20, "слово целиком")
            add_xp(uid, 15)
            update_stat(uid, "wins")
            with game_lock:
                active_games.pop(wk, None)
...
def on_sticker(msg):
    try:
        ...
        if random.randint(1, 100) <= chance:
            ...
            r = ask_ai([{"role": "system", "content":
                f"Ты Хината. Стикер {emoji}. ОДНА короткая фраза. БЕЗ скобок."},
                {"role": "user", "content": f"[стикер {emoji}]"}], max_tokens=60)
            if r and not is_err(r):
                resp = clean(r)
...
def on_voice(msg):
    try:
        ...
        if is_reply_to_bot or random.randint(1, 100) <= chance:
            r = ask_ai([{"role": "system", "content":
                "Ты Хината. Голосовое. Не можешь послушать. 1 короткая фраза."},
                {"role": "user", "content": "[голосовое]"}], max_tokens=60)
            if r and not is_err(r):
                resp = clean(r)
                if resp:
                    safe_send(cid, resp, reply_to=msg.message_id)
...
def on_photo(msg):
    try:
        ...
        if is_reply_to_bot or is_mention or random.randint(1, 100) <= chance:
            cap = msg.caption or ""
            # Попытка через vision модель — в отдельном потоке
            def _analyze():
                try:
                    photo = msg.photo[-1]
                    ...
                    r = requests.post("https://openrouter.ai/api/v1/chat/completions",
                        headers={"Authorization": f"Bearer {OPENROUTER_API_KEY}",
                                 "Content-Type": "application/json"},
                        json={"model": current, "messages": [
                            {"role": "system", "content":
                                "Ты Хината. Прокомментируй фото. 1 короткая фраза. БЕЗ скобок."},
                            {"role": "user", "content": [
                    ...
                    if r.status_code == 200:
                        resp = clean(r.json().get("choices", [{}])[0]
                ...
                # Фоллбэк
                r = ask_ai([{"role": "system", "content":
                    "Ты Хината. Фото. 1 короткая фраза."},
                    {"role": "user", "content": cap or "[фото]"}], max_tokens=60)
                if r and not is_err(r):
                    resp = clean(r)
                    if resp:
                        safe_send(cid, resp, reply_to=msg.message_id)
...
def on_gif(msg):
    try:
        ...
        if random.randint(1, 100) <= (30 if is_pm(msg) else 8):
            r = ask_ai([{"role": "system", "content":
                "Ты Хината. Гифка. 1 короткая фраза."},
                {"role": "user", "content": "[гифка]"}], max_tokens=60)
            if r and not is_err(r):
                resp = clean(r)
                if resp:
                    safe_send(cid, resp, reply_to=msg.message_id)
    ...
    try:
        ...
        if is_pm(msg):
            ...
            if is_msearch:
                ...
                if not found:
                    safe_reply(msg, f"Ничего по '{query}'. /dev_model")
                elif len(found) == 1:
                    global CURRENT_MODEL
                    with model_lock:
                    ...
                    save_bot_state()
                    minfo = found[0][1]
                    safe_reply(msg, f"✅ {minfo['name']} ({'🆓' if minfo.get('free') else '💰'})")
                else:
                    kb = types.InlineKeyboardMarkup(row_width=1)
                    for mid_key, minfo in found[:10]:
                        with model_lock:
                            current = CURRENT_MODEL
                        mark = "✅ " if minfo["id"] == current else ""
                        free = "🆓" if minfo.get("free") else "💰"
                        kb.add(types.InlineKeyboardButton(
                            f"{mark}{free} {minfo['name']}", callback_data=f"mset_{mid_key}"))
                    kb.row(types.InlineKeyboardButton("◀", callback_data="mcat_back"))
            ...
            if is_ct:
                ...
                save_prof(uid, {"custom_title": title})
            ...
            if is_ne:
                ...
                save_prof(uid, {"name_emoji": emoji})
        ...
        if is_pm(msg) and uid in secret_links:
            ...
            resp = ask_ai(msgs, max_tokens=300)
            if is_err(resp):
        ...
        if is_grp(msg):
            with user_states_lock:
                state = user_states.pop(f"{cid}_{uid}", None)
            if state == "wp":
                if msg.text.lower().strip() == "отмена":
        ...
        if ts.isdigit() and 1 <= int(ts) <= 8:
            ...
            if pl:
                ...
                if num < len(lv.get("results", [])):
                    ...
                    set_busy(cid, "music", track['title'])
                    smsg = safe_send(cid, f"качаю {track['title'][:35]}... 🎵",
                                     reply_to=msg.message_id)
                    if not smsg:
                        clear_busy(cid)
                        return
        ...
        def get_max_tokens(text, is_group):
            if is_group:
                return 200  # Группа: коротко
            # ЛС: зависит от сложности
            if len(text) > 200 or any(w in text.lower() for w in
                                       ["расскажи", "объясни", "напиши", "история", "стих"]):
                return 600
            return 300
        ...
        if is_pm(msg):
            ...
            mt = get_max_tokens(msg.text, False)
            resp = ask_ai(msgs, max_tokens=mt)
            if is_err(resp):
        ...
        mt = get_max_tokens(msg.text, True)
        resp = ask_ai(msgs, max_tokens=mt)
        if is_err(resp):
            ...
            if os.path.exists(DOWNLOADS_DIR):
                for item in os.listdir(DOWNLOADS_DIR):
                    ...
                    try:
                        ...
                        pass
            cleanup_pending()
            # Очистка pending mod actions (старше 10 мин)
            with mod_lock:
                for k in [k for k, v in pending_mod_actions.items()
                          if v.get("time") and (datetime.now() - v["time"]).total_seconds() > 600]:
                    pending_mod_actions.pop(k, None)
            with game_lock:
...
if __name__ == "__main__":
    ...
    free_count = sum(1 for m in AVAILABLE_MODELS.values() if m.get("free"))
    paid_count = sum(1 for m in AVAILABLE_MODELS.values() if not m.get("free"))
    log.info(f"Моделей: {len(AVAILABLE_MODELS)} (🆓{free_count} / 💰{paid_count})")
    log.info(f"БД: SQLite ({DB_FILE})")
