...
# === МОДЕЛИ ===
AVAILABLE_MODELS = {
    "gemini-flash": {"id": "google/gemini-2.0-flash-001", "name": "Gemini 2.0 Flash", "free": True},
    "gemini-pro": {"id": "google/gemini-pro", "name": "Gemini Pro", "free": True},
    "llama-70b": {"id": "meta-llama/llama-3-70b-instruct", "name": "Llama 3 70B", "free": True},
    "llama-8b": {"id": "meta-llama/llama-3-8b-instruct", "name": "Llama 3 8B", "free": True},
    "mixtral": {"id": "mistralai/mixtral-8x7b-instruct", "name": "Mixtral 8x7B", "free": True},
    "qwen-72b": {"id": "qwen/qwen-2-72b-instruct", "name": "Qwen 2 72B", "free": True},
    "phi-3": {"id": "microsoft/phi-3-medium-128k-instruct", "name": "Phi 3 Medium", "free": True},
    "gemma-27b": {"id": "google/gemma-2-27b-it", "name": "Gemma 2 27B", "free": True},
    "deepseek": {"id": "deepseek/deepseek-chat", "name": "DeepSeek V2", "free": True},
    "command-r": {"id": "cohere/command-r", "name": "Command R", "free": True},
}

CURRENT_MODEL = os.environ.get("MODEL_ID", "google/gemini-2.0-flash-001")
BOT_NAME = "Хината"
...
active_games = {}
reminders = {}
secret_links = {}  # uid -> group_id для секретного управления
...
def is_admin(cid, uid):
    ...
    if str(uid) in s.get("admins", {}):
        return True
    # Авто-админ: проверяем права в Telegram
    if s.get("auto_admin"):
        try:
...
def record_gift(from_uid, from_name, gift_item):
    """Записываем подарок в историю Хинаты"""
    gifts = load_gifts(0)  # 0 = Хината
...
def add_to_pl(uid, track, group_cid=None):
    pl = load_pl(uid)
    for t in pl["tracks"]:
    ...
    # Добавляем и в общий плейлист группы
    if group_cid:
...
# ================= МОДЕРАЦИЯ =================
def do_mod_action(cid, action_text):
    ...
    for uk, info in mem.get("users", {}).items():
        ...
        for field in ["tg_name", "name", "preferred_name"]:
            v = info.get(field, "")
            if v and target.lower() in v.lower():
                try:
                    target_uid = int(uk)
                except Exception:
                    pass
    ...
    if not target_uid:
        return f"не нашла {target} в чате"
...
DOCS = {
    ...
    "media": (
        ...
        "После скачивания появится кнопка 💾\n"
        "для сохранения в плейлист.\n"
    ),
    "games": (
        "🎮 ИГРЫ\n\n"
        "Команда: /game\n"
    ...
    "playlist": (
        ...
        "Добавить: кнопка 💾 после скачивания\n"
        "Удалить: /plremove [номер]\n"
        "Рандом: «Хината, трек из плейлиста»\n\n"
        "В группе треки добавляются\n"
        "и в личный и в общий плейлист.\n"
    ),
}
...
HELP_VOICE = (
    ...
    "• «размуть [имя]»\n\n"
    "📋 Другое:\n"
    "• «что обсуждали?» — саммари\n"
    "• «расскажи про [тему]» — поиск\n\n"
    "Разработчик: @PaceHoz\n"
)
...
def cmd_dev_model(msg):
    ...
    if len(parts) < 2:
        t = f"🤖 Текущая: {CURRENT_MODEL}\n\nДоступные:\n"
        for k, v in AVAILABLE_MODELS.items():
            t += f"• {k} — {v['name']} {'🆓' if v['free'] else '💰'}\n"
        t += "\n/dev_model [название]"
    ...
    if key in AVAILABLE_MODELS:
        ...
        safe_send(msg.chat.id, "Не найдена")
...
def handle_save_cb(call, uid, cid, mid):
    ...
    saved = sum(1 for t in lv.get("results", []) if add_to_pl(uid, t, cid if cid < 0 else None))
...
def handle_plsv_cb(call, uid, cid, mid, data):
    with user_states_lock:
        track = user_states.pop(data, None)
    if not track:
        bot.answer_callback_query(call.id, "⏰", show_alert=True)
        safe_delete(cid, mid)
        return
    if add_to_pl(uid, track, cid if cid < 0 else None):
        bot.answer_callback_query(call.id, "💾 Сохранено!", show_alert=True)
        safe_edit(f"💾 {track['title']} — в плейлисте!", cid, mid)
        achs = check_achs(uid)
        notify_achs(cid, uid, achs)
    else:
...
def handle_pm_cb(call, uid, cid, mid, data):
    ...
    elif data == "balance" or data == "stats":
        p = load_prof(uid)
    ...
    try:
        ...
        try:
            ...
            if req_uid:
                ...
                sk = f"plsv_{cid}_{req_uid}_{int(time.time())}"
                with user_states_lock:
                    user_states[sk] = ti
                kb = types.InlineKeyboardMarkup()
                kb.add(types.InlineKeyboardButton("💾 В плейлист", callback_data=sk))
    ...
    for action in actions:
        if action["type"] == "music_search":
            ...
            if not s.get("moderation"):
                ...
                continue
            # ТОЛЬКО владелец, админы и разработчик
            if uid and (is_owner(cid, uid) or is_admin(cid, uid) or uid in DEVELOPER_IDS):
                result = do_mod_action(cid, action["action"])
...
def on_voice(msg):
    try:
        ...
        bi = get_bot_info()
        is_reply = msg.reply_to_message and bi and msg.reply_to_message.from_user and msg.reply_to_message.from_user.id == bi.id
...
def on_photo(msg):
    try:
        ...
        bi = get_bot_info()
        is_reply = msg.reply_to_message and bi and msg.reply_to_message.from_user and msg.reply_to_message.from_user.id == bi.id
        is_mention = msg.caption and (is_named(msg.caption) or (bi and bi.username and f"@{bi.username.lower()}" in msg.caption.lower()))
            ...
            try:
                ...
                r = requests.post("https://openrouter.ai/api/v1/chat/completions",
                    ...
                    json={"model": CURRENT_MODEL, "messages": [
                        ...
                        {"role": "user", "content": [{"type": "text", "text": cap or "что скажешь?"}, {"type": "image_url", "image_url": {"url": furl}}]}
    ...
    try:
        ...
        if is_pm(msg) and uid in secret_links:
            ...
            if is_err(resp):
                ...
                return
            ct, actions = parse_actions(resp)
            ct = clean(ct)
            if ct:
                add_msg(target_gid, "assistant", ct, True)
                safe_send(target_gid, ct)
        ...
        if is_pm(msg):
            ...
            ct = clean(ct)
            if ct:
                add_msg(uid, "assistant", ct)
                send_long(cid, ct, markup=main_kb(), reply_to=msg.message_id)
            if actions:
        ...
        direct = is_reply or is_mention or is_named(msg.text)
        ...
        ct = clean(ct)
        if ct:
            add_msg(cid, "assistant", ct, True)
            send_long(cid, ct, reply_to=msg.message_id)
        if actions:
