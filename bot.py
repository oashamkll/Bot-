...
# === МОДЕЛИ (расширенный список, сортированы: free → paid) ===
AVAILABLE_MODELS = {
    # ===== БЕСПЛАТНЫЕ =====
    # Google
...
def ask_ai(messages):
    try:
        ...
        if r.status_code == 403:
            return "[ERR]модель недоступна (403), попробуй другую 🤔"
...
def model_cats_kb():
    """Кнопки категорий моделей — сортировка: бесплатные сверху, платные снизу"""
    kb = types.InlineKeyboardMarkup(row_width=2)
    cats_free = {}
    cats_paid = {}
    for mid, minfo in AVAILABLE_MODELS.items():
        ...
        if minfo.get("free"):
            if cat not in cats_free:
                cats_free[cat] = 0
            cats_free[cat] += 1
        else:
            if cat not in cats_paid:
                cats_paid[cat] = 0
            cats_paid[cat] += 1

    # Сначала бесплатные категории
    if cats_free:
...
def model_list_kb(cat, free_filter=None):
    """Кнопки моделей в категории с фильтром free/paid"""
    kb = types.InlineKeyboardMarkup(row_width=1)
    with model_lock:
        current = CURRENT_MODEL
    for mid, minfo in AVAILABLE_MODELS.items():
...
def cmd_dev_model(msg):
    ...
    if key in AVAILABLE_MODELS:
        ...
        safe_send(msg.chat.id, f"✅ Модель: {minfo['name']} ({'🆓' if minfo.get('free') else '💰'})\n{minfo['id']}")
    elif key.count("/") == 1:
        with model_lock:
            CURRENT_MODEL = key
        save_bot_state()
...
def handle_model_cb(call, uid, cid, mid, data):
    ...
    if data == "mcat_search":
        ...
        safe_edit("🔍 Напиши название модели или часть (например: llama, gemini, gpt, solar):", cid, mid)
        return

    # mcat_{category}_{free|paid}
    if data.startswith("mcat_"):
        ...
        if len(parts) == 2 and parts[1] in ("free", "paid"):
            ...
            free_filter = None
        cat_name = MODEL_CATEGORIES.get(cat, cat)
        filter_label = " 🆓" if free_filter == "free" else " 💰" if free_filter == "paid" else ""
        t = f"{cat_name}{filter_label}\n\n✅ = текущая | 🆓 = бесплатная | 💰 = платная\n"
        safe_edit(t, cid, mid, markup=model_list_kb(cat, free_filter))
        return
    ...
    try:
        ...
        if is_pm(msg):
            ...
            if is_msearch:
                ...
                found = []
                for mid_key, minfo in AVAILABLE_MODELS.items():
                    if query in mid_key.lower() or query in minfo["name"].lower() or query in minfo["id"].lower():
                        found.append((mid_key, minfo))
                if not found:
                    ...
                    for mid_key, minfo in found[:10]:
                        ...
                        kb.add(types.InlineKeyboardButton(
                            f"{current_mark}{free} {minfo['name']}", callback_data=f"mset_{mid_key}"))
                    kb.row(types.InlineKeyboardButton("◀ Назад", callback_data="mcat_back"))
